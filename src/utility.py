...
def get_file_metadata(file_path):
    ...
    metadata = {
        ...
        "file_type": ''.join(Path(file_path).suffixes) if S_ISREG(file_stats.st_mode) and mt[0] else None,
        "file_encode": mt[1] if S_ISREG(file_stats.st_mode) else None,
        "file_size": file_stats.st_size if S_ISREG(file_stats.st_mode) else None,  # in bytes
        "modified_time": datetime.fromtimestamp(file_stats.st_mtime).strftime('%Y-%m-%d %H:%M:%S'),
        "file_permission": filemode(file_stats.st_mode),
        "DIRorREGFILE": "DIR" if S_ISDIR(file_stats.st_mode) else "REGFILE",
